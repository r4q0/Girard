...
An installed, local-first helper for Josef's scope: capture **call playback**, turn it into live English text, and keep capture/processing latency and API cost visible. It does not capture your microphone, send text to an LLM, perform emotion detection, or create sales reports.

## Start using it

Open **Call Audio** on the Desktop or in the application menu. If your desktop asks, allow this launcher to run. It opens a terminal and the local page at **http://127.0.0.1:8765**. Alternatively:

```bash
/home/JP/Projects/call-audio-helper/launch.sh
```
...
The computer's physical output was already muted during setup; this installer leaves mute/volume unchanged. Unmute in your desktop controls when you want to hear a call. A muted output can affect monitor capture depending on the audio-server setup; the playback meter is your check. Isolated capture is taken before the physical output.
...
From `/home/JP/Projects/call-audio-helper`:
...
Original research plans remain unchanged in `/home/JP/Desktop/Audio Plans/`.
