# Demo call script (1 minute)

The rep sells for Koref (the salesbook Girard loads). The prospect is Sanne, operations manager at an installation company.

## Setup

- Two devices on one call. The rep uses the laptop running Girard; the prospect talks from the other device.
- On the Girard laptop, pick the output the call plays on under **Audio device**, then press **Start call**.
- Girard only hears the prospect. The rep's microphone is never captured.
- **Prospect:** say each line as one clear sentence, then pause about one second.
- **Rep:** answer from the openers on the card.
...
Each prospect line was run through Girard in this order and gave the card in the table.
